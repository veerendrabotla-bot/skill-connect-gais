...
import { 
  ...
  Gavel, Wallet, Info, Lock, Activity
} from 'lucide-react';
...
const AuditComplianceView: React.FC = () => {
  ...
  const handleExport = async () => {
    setExporting(true);
    // Simulate data aggregation for export
  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
         ...
         <div className="bg-indigo-900 text-white p-8 rounded-[3rem] shadow-xl space-y-4 relative overflow-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                  ...
                  <div className="flex items-center gap-4">
                     <div className="relative w-64">
                        ...
                        <input 
                           ...
                           className="w-full pl-11 pr-4 py-2.5 bg-gray-50 border border-transparent focus:border-indigo-600 rounded-xl text-xs font-bold transition-all"
                        />
                     </div>
                     <button onClick={handleExport} disabled={exporting} className="p-2.5 bg-indigo-600 text-white rounded-xl hover:bg-black transition-all">
         ...
         <div className="lg:col-span-4 space-y-10">
            {selectedLog ? (
               <div className="bg-white p-10 rounded-[3.5rem] border border-gray-100 shadow-sm space-y-8 animate-in slide-in-from-right-10 duration-500">
                  <div className="flex items-center justify-between">
                     <h4 className="text-xl font-black text-gray-900 uppercase tracking-tight">Dossier Inspector</h4>
                     <button onClick={() => setSelectedLog(null)} className="text-[10px] font-black text-gray-400 uppercase tracking-widest hover:text-red-500">Close</button>
                  </div>
                  
                  <div className="space-y-6">
                     <div className="space-y-2">
                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Event Fingerprint</p>
                        <div className="p-4 bg-gray-50 rounded-2xl border border-gray-100 font-mono text-[9px] break-all text-gray-600">
                           {selectedLog.id}
                        </div>
                     </div>

                     <div className="space-y-4">
                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Mutation Payload</p>
                        <div className="bg-indigo-950 p-6 rounded-[2rem] shadow-inner max-h-[400px] overflow-y-auto">
                     ...
                     <div className="bg-blue-50 p-6 rounded-3xl border border-blue-100 flex items-start gap-4">
                        ...
                        <p className="text-[10px] text-blue-900 font-bold leading-relaxed uppercase tracking-tight">
                           This mutation was signed by {selectedLog.actor_name} on {new Date(selectedLog.created_at).toLocaleString()}.
                        </p>
                     </div>
                  </div>
               </div>
            ) : (
               <div className="bg-white p-12 rounded-[4rem] border-4 border-dashed border-gray-50 text-center flex flex-col items-center justify-center space-y-6 min-h-[400px]">
                  <div className="w-20 h-20 bg-gray-50 rounded-full flex items-center justify-center text-gray-200">
                     <Search size={40} />
                  </div>
                  <div className="space-y-2">
                     <h4 className="text-xl font-black text-gray-300 uppercase tracking-widest">Inspector Idle</h4>
                     <p className="text-xs text-gray-400 font-medium max-w-[200px] mx-auto">Select a ledger entry to view forensic details.</p>
                  </div>
               </div>
            )}
            ...
            <div className="bg-indigo-900 text-white p-10 rounded-[3.5rem] shadow-3xl space-y-6 relative overflow-hidden group">
               ...
               <div className="flex items-center gap-4 relative z-10">
                  ...
                  <h4 className="text-lg font-black tracking-tight uppercase">Security Terminal</h4>
               </div>
               <p className="text-xs text-indigo-300 font-medium leading-relaxed italic relative z-10">
                  "Compliance archives are retained for 7 years as per Platform Governing Policy v2.1. Forensic auditing is strictly restricted to Level 4 Administrators."
               </p>
            </div>
         </div>
      </div>
