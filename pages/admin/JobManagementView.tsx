...
const JobManagementView: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Reactive Mesh Subscription
    const subscription = adminService.subscribeToTable('jobs', () => {
      console.debug("Mesh Update Detected: Re-syncing grid...");
      loadJobs();
    });
  ...
  if (selectedJob) {
    const dispatcherData = selectedJob.metadata;
    return (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                   ...
                   <div className="space-y-6 bg-gray-50 p-8 rounded-[3rem] border border-gray-100 shadow-inner">
                      ...
                      {selectedJob.worker_name ? (
                        ...
                        <p className="text-sm font-bold text-gray-400 italic">Dispatcher Optimizing...</p>
                      )}
                   </div>
                </div>

                {/* Dispatcher Intelligence PEER VIEW */}
                {selectedJob.status === 'MATCHING' && dispatcherData?.candidates && (
                  <div className="bg-indigo-950 text-white p-10 rounded-[3.5rem] space-y-8 animate-in zoom-in border border-white/10 shadow-3xl">
                     <div className="flex items-center justify-between">
                        <div className="space-y-1">
                           <h4 className="text-2xl font-black uppercase tracking-tight flex items-center gap-3">
                              <Sparkles className="text-indigo-400" /> Dispatcher Brain
                           </h4>
                           <p className="text-[10px] text-indigo-400 font-black uppercase tracking-[0.3em]">AI-Driven Candidate Ranking</p>
                        </div>
                        <div className="px-4 py-1.5 bg-white/10 rounded-xl text-[9px] font-black uppercase tracking-widest border border-white/10">Active Simulation</div>
                     </div>
                     
                     <div className="space-y-4">
                        {dispatcherData.candidates.map((cand: any, i: number) => (
                           <div key={cand.id} className="flex items-center justify-between p-6 bg-white/5 rounded-3xl border border-white/5 group hover:bg-white/10 transition-all">
                              <div className="flex items-center gap-6">
                                 <div className="w-10 h-10 bg-indigo-600 rounded-xl flex items-center justify-center font-black text-sm">{i+1}</div>
                                 <div>
                                    <p className="text-sm font-black uppercase tracking-tight">{cand.full_name}</p>
                                    <p className="text-[9px] font-bold text-indigo-300 uppercase tracking-widest">Score: {cand.rating} • Dist: {(cand.dist_m / 1000).toFixed(1)}km</p>
                                 </div>
                              </div>
                              <Navigation size={18} className="text-indigo-500 group-hover:translate-x-1 transition-transform" />
                           </div>
                        ))}
                     </div>
                  </div>
                ...
                <div className="space-y-6">
                   ...
                   <div className="space-y-6 pl-4 border-l-2 border-gray-50">
                      {loadingDetails ? (
                        ...
                        <p className="text-sm font-bold text-gray-300 uppercase">Genesis Event Logs Unavailable.</p>
                      )}
                   </div>
                </div>
             </div>
          ...
          <div className="lg:col-span-4 space-y-10">
             <div className="bg-white p-10 rounded-[3.5rem] border border-gray-100 shadow-sm space-y-10">
                <div className="space-y-4">
                   ...
                   <textarea 
                     ...
                     placeholder="State the administrative reason for this override..."
                     rows={5}
             ...
             <div className="bg-indigo-900 text-white p-10 rounded-[3.5rem] shadow-3xl space-y-6 relative overflow-hidden">
                ...
                <div className="flex items-center gap-4 relative z-10">
                   ...
                   <h4 className="text-lg font-black tracking-tight uppercase">Audit Lock</h4>
                </div>
                <p className="text-xs text-indigo-300 font-medium leading-relaxed italic relative z-10">
                   "Overrides are irreversible and bypass the standard settlement handshake. Use only for site emergencies or fraud prevention."
                </p>
             </div>
          </div>
        </div>
    ...
    <div className="space-y-12 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
         <div className="space-y-1">
            ...
            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Real-time telemetry of all active sector nodes</p>
         </div>
            ...
            <table className="w-full text-left">
               ...
               <tbody className="divide-y divide-gray-50">
                  {loading && jobs.length === 0 ? (
                    ...
                    return (
                      <tr key={job.id} className="hover:bg-gray-50/50 transition-colors group animate-in fade-in">
                         ...
                         <td className="px-10 py-8">
                            <div className="space-y-1">
                               <p className="text-xs font-black text-gray-900 uppercase tracking-tight">Cust: {job.customer_name}</p>
                               <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">Work: {job.worker_name || 'IDENTIFYING'}</p>
                            </div>
                         </td>
                         <td className="px-10 py-8">
                    ...
                    <tr>
                       <td colSpan={5} className="p-20 text-center">
                          ...
                          <p className="text-gray-400 font-black uppercase tracking-widest text-xs">Zero sector nodes detected in the current mesh.</p>
                       </td>
                    </tr>
                  )}
               </tbody>
