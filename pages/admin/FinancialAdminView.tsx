...
import { 
  ...
  Briefcase, Landmark as BankIcon, BarChart3
} from 'lucide-react';
import { 
             ...
             <div className="bg-white p-10 sm:p-14 rounded-[4.5rem] border border-gray-100 shadow-sm space-y-12">
                <div className="flex items-center justify-between">
                   <div className="space-y-1">
                      ...
                      <p className="text-xs text-gray-400 font-bold uppercase">{selectedPayout.worker_email}</p>
                   </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                   <div className="space-y-6 bg-gray-50 p-8 rounded-[3rem] border border-gray-100">
                      ...
                      <div className="space-y-3">
                         <div className="flex justify-between text-xs">
                            <span className="text-gray-400 font-bold">Institution:</span>
                         ...
                         <div className="flex justify-between text-xs">
                            <span className="text-gray-400 font-bold">IFSC Protocol:</span>
                   ...
                   <div className="space-y-6 bg-indigo-900 text-white p-8 rounded-[3rem] shadow-xl relative overflow-hidden">
                      ...
                      <p className="text-[9px] text-indigo-300/60 font-bold uppercase italic relative z-10">Post-payout balance: ₹{selectedPayout.current_vault_balance - selectedPayout.amount}</p>
                   </div>
                </div>
                      ...
                      <div className="overflow-x-auto">
                        <table className="w-full text-left">
                           <thead className="bg-gray-50/50">
                              <tr>
                                 <th className="px-8 py-4 text-[9px] font-black text-gray-400 uppercase">Timestamp</th>
                                 <th className="px-8 py-4 text-[9px] font-black text-gray-400 uppercase">Description</th>
                                 <th className="px-8 py-4 text-[9px] font-black text-gray-400 uppercase text-right">Settlement</th>
                              </tr>
                           </thead>
                           <tbody className="divide-y divide-gray-50">
                              {loadingLedger ? (
                                 <tr><td colSpan={3} className="p-10 text-center"><Loader2 className="animate-spin mx-auto text-gray-200" /></td></tr>
                                 ...
                                 <tr key={tx.id} className="text-[10px]">
                                    ...
                                    <td className={`px-8 py-4 text-right font-black ${tx.type === 'CREDIT' ? 'text-green-600' : 'text-red-500'}`}>
                                       {tx.type === 'CREDIT' ? '+' : '-'} ₹{Math.abs(tx.amount)}
                                    </td>
                                 </tr>
                              )) : (
                                <tr><td colSpan={3} className="p-10 text-center text-gray-400 font-bold uppercase italic">No ledger history available.</td></tr>
                              )}
                           </tbody>
                        </table>
                      </div>
  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
         ...
         <div className="bg-indigo-600 text-white p-8 rounded-[3rem] shadow-xl space-y-4">
         ...
         <div className="flex bg-white p-1 rounded-2xl border border-gray-100 shadow-sm w-fit">
            {(['PAYOUTS', 'TREASURY', 'REVENUE'] as const).map(tab => (
               <button
                  ...
                  className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeSubTab === tab ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-100' : 'text-gray-400 hover:text-gray-900'}`}
               >
                  {tab}
               </button>
            ))}
            ...
            {loading ? (
               ...
               >
                  ...
                  <div className="space-y-1">
                     ...
                     <p className="text-xs text-gray-400 font-bold line-clamp-1 italic uppercase tracking-tight">{p.bank_details?.bankName || 'Unknown Bank'}</p>
                  </div>
               ...
               <div className="col-span-full py-40 text-center flex flex-col items-center">
                  ...
                  <h3 className="text-3xl font-black text-gray-300 uppercase tracking-widest">Settlements Optimized</h3>
                  <p className="text-gray-400 font-medium mt-2 italic">Zero pending payout requests detected in the fiscal pipeline.</p>
               </div>
            )}
         </div>
      )}
            ...
            <div className="bg-white p-12 sm:p-16 rounded-[4rem] border border-gray-100 shadow-sm space-y-12">
               <div className="flex items-end justify-between">
                  <div className="space-y-2">
                     <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.3em]">Fiscal Telemetry</h4>
                     <h3 className="text-3xl font-black text-gray-900 tracking-tighter uppercase">Platform Volume vs. Yield</h3>
                  </div>
                  <div className="flex gap-3 items-center bg-gray-50 px-5 py-2.5 rounded-2xl border border-gray-100">
                     <BarChart3 size={18} className="text-indigo-600" />
                     <span className="text-[10px] font-black text-gray-400 uppercase">30-Day Window</span>
                  </div>
               </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
               <div className="bg-indigo-900 text-white p-12 rounded-[4rem] shadow-2xl relative overflow-hidden">
                  ...
                  <h4 className="text-[10px] font-black text-indigo-300 uppercase tracking-widest mb-6 flex items-center gap-2">
                     <ShieldCheck size={16} /> Regulatory Reserves
                  </h4>
                  <div className="space-y-2">
                  ...
                  <p className="text-[10px] text-indigo-300/60 font-bold uppercase mt-8 leading-relaxed italic">
                     Funds held in temporary compliance vault for quarterly remittance simulations.
                  </p>
               </div>
               <div className="bg-white p-12 rounded-[4rem] border border-gray-100 shadow-sm space-y-8 flex flex-col justify-center">
                  <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-2">
                     <PieChart size={16} className="text-emerald-500" /> Platform P&L Index
                  </h4>
                  <div className="grid grid-cols-2 gap-8">
                     <div>
                        <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">Margin</p>
         ...
         <div className="bg-white rounded-[4rem] border border-gray-100 shadow-sm overflow-hidden">
            <div className="overflow-x-auto">
               <table className="w-full text-left">
                  <thead className="bg-gray-50/50">
                     <tr>
                        <th className="px-10 py-6 text-[10px] font-black text-gray-400 uppercase">Timestamp</th>
                        <th className="px-10 py-6 text-[10px] font-black text-gray-400 uppercase">Gross Payment</th>
                        <th className="px-10 py-6 text-[10px] font-black text-gray-400 uppercase">Comm %</th>
                        <th className="px-10 py-6 text-[10px] font-black text-gray-400 uppercase">GST Component</th>
                        <th className="px-10 py-6 text-[10px] font-black text-gray-400 uppercase text-right">Net Yield</th>
                     </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-50">
                     {revenueLogs.map(log => (
